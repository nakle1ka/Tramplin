...
func (r *employerRepository) Update(ctx context.Context, employer *Employer) error {
	return r.getDB(ctx).Save(employer).Error
}

func (r *employerRepository) Delete(ctx context.Context, id uuid.UUID) error {
	return r.getDB(ctx).Delete(&Employer{}, "id = ?", id).Error
}
