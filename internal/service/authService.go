...
func (s *authService) Refresh(ctx context.Context, refreshToken string) (AuthResult, error) {
	...
	err = s.txManager.Wrap(ctx, func(txCtx context.Context) error {
		...
		accessToken, newRefreshToken, err := s.issueSession(userID)
		if err != nil {
			return err
		}
...
func (s *authService) issueSession(userID uuid.UUID) (string, string, error) {
	accessToken, _, err := s.tokenManager.GenerateToken(userID.String(), s.accessExp)
	if err != nil {
		return "", "", fmt.Errorf("generate access: %w", err)
	}

	refreshToken, tokenID, err := s.tokenManager.GenerateToken(userID.String(), s.refreshExp)
	if err != nil {
...
func validateEmployerINN(inn string) (bool, error) {
	n := len(inn)
	if n != 10 && n != 12 {
		return false, errors.New("ИНН работодателя должен быть 10 или 12 цифр")
	}

	digits := make([]int, n)
	for i := range n {
		if inn[i] < '0' || inn[i] > '9' {
			return false, errors.New("недопустимый символ")
		}
	...
	if n == 10 {
		...
		return digits[9] == check(digits, w10), nil
	} else {
